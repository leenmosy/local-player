...
(function blockMobileDevices(){
  ...
  function checkDeviceBlock(){
    const coarseOnly = coarseMQL.matches && !fineMQL.matches && Math.min(window.innerWidth, window.innerHeight) <= 1024;
...
async function tryRestoreFolderPlaylist(folderId, activeFile, activeHandle, loadMeta){
  ...
  for (const meta of manifest.files){
    ...
    const key = FOLDER_PROGRESS_PREFIX + meta.name + ':' + meta.size + ':' + (meta.lastModified || 0);
    try{
...
function loadFile(file, handle, meta){
  ...
  currentFileKey = fileKey(file, currentFileIsFolder);
...
// Определяет по названию главы, относится ли она к пропускаемому сегменту,
// и подбирает текст для плашки + "тип" (нужен для потолка длительности ниже).
//
// Два независимых способа пометить главу как пропускаемую:
//
// 1) ТОЧНОЕ совпадение с одним из канонических слов (без учёта регистра).
//    Это не самопальный список ключевых слов "где угодно в названии" (как
//    было раньше) — название должно РАВНЯТЬСЯ одному из этих слов целиком.
//    Набор ("Intro"/"Credits"/"Recap"/"Preview" и русские аналоги) выбран
//    не случайно: это де-факто конвенция, которую уже используют другие
//    плееры/плагины для домашних медиа-серверов (Jellyfin, Plex-плагины
//    вроде intro-skipper) — единого формального стандарта тут нет (сервисы
//    вроде Netflix хранят такие тайминги у себя на бэкенде, а не в файле),
//    но это самое близкое к общепринятой практике решение, и файлы,
//    размеченные так, будут вести себя ожидаемо и в других плеерах тоже.
//    Именно поэтому это ТОЧНОЕ совпадение, а не подстрока: глава с названием
//    "Титры к части 1" не должна случайно попасть под "титры".
//
// 2) Явный префикс "SKIP:" — запасной вариант для всего, что не входит в
//    канонический список (реклама, спонсорская вставка, любой язык,
//    произвольная метка). Тут можно писать что угодно.
//
// Абсолютно любая другая глава (в т.ч. названная как угодно иначе, хоть
// "Материал", хоть "Часть 2") безопасна и просто закрывает предыдущий
// пропускаемый отрезок — как ей и положено по формату mp4-глав.
...
function applyChaptersFromMediaInfoResult(result, token){
  ...
  // Потолок длительности — это АВАРИЙНЫЙ тормоз, а не "типичная длина
  // интро/рекапа". Он не должен мешать корректно размеченным отрезкам
  // (например, реальное интро на 3:20 с явно поставленной закрывающей
  // главой — это нормально, кап тут вообще не должен участвовать).
  // Возможная ситуация, от которой он защищает: пользователь пометил
  // "Intro", но забыл поставить следующую главу рядом — тогда разрыв до
  // следующей найденной главы может составлять хоть 40, хоть 90 минут
  // (или вообще Infinity, если это последняя глава файла). Именно такие
  // аномально большие разрывы кап и обрезает, не трогая нормальные случаи.
  // У титров кап не нужен вообще — им законно можно идти до конца видео.
...
function openFolderPlaylist(items, folderName){
  ...
  playlistFolderId = computeFolderId(playlistFiles);
  playlistBtn.style.display = playlistFiles.length > 1 ? '' : 'none';
  renderPlaylist();
  updatePlaylistNavButtons();
  ...
  playlistFiles.forEach(entry => {
    if (entry.handle){
      idbSet(fileKey(entry.file, true), entry.handle).catch(() => {});
    }
  });
...
function retryWithoutCrossOriginOnError(url, thisLoadToken, onRecovered){
  ...
  video.addEventListener('error', urlErrorHandler = function(){
    ...
    showUrlError(fallback, { duration: 8000 });
    diagnoseVideoLoadError(url, fallback).then(msg => {
      if (thisLoadToken !== urlLoadToken) return;
      if (msg !== fallback) showUrlError(msg, { duration: 8000 });
    });
  }, { once: true });
  return true;
}
...
function loadUrl(url){
  ...
  if (isM3U8 && typeof Hls !== 'undefined' && Hls.isSupported()){
    ...
    video.addEventListener('error', urlErrorHandler = function(){
      ...
      urlLoadBtn.disabled = false;
      if (isM3U8){
        showUrlError(' Браузер не поддерживает m3u8', { duration: 8000 });
      } else {
        const fallback = ' Не удалось загрузить видео';
        showUrlError(fallback, { duration: 8000 });
        diagnoseVideoLoadError(url, fallback).then(msg => {

...
// Определяет по названию главы, относится ли она к пропускаемому типу
// контента (заставка/интро или титры), и подбирает текст для плашки.
// Если название не распознано — глава считается обычной и никак не влияет
// на плеер (это может быть, например, глава "Часть 2" — её мы не трогаем).
...
function applyChaptersFromMediaInfoResult(result, token){
  ...
  for (let i = 0; i < dedup.length; i++){
    ...
    const end = i + 1 < dedup.length ? dedup[i + 1].time : Infinity; // Infinity — "до конца видео"
